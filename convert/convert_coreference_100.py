...
coreference = {'version': 'sync_part', 'data': []}
        ...
        with open(read_data_file, 'w', encoding='utf-8') as fout:
            for id, attribute in data.items():
                ...
                if id in dict:
                    ...
                    qp_pair['query_sync_tokens'] = [i + len(attribute['question_tokens']) for i in
                                                    qp_pair['query_sync_tokens']]
                    fout.write(str(qp_pair['query_sync_tokens']) + ' ')
                    fout.write(str([attribute['paragraph'].split()[i - len(qp_pair['question'].split())] for i in
                                    qp_pair['query_sync_tokens']]))
                    fout.write('\n')
                    qp_pair['paragraph'] = attribute['paragraph']
                    fout.write(qp_pair['paragraph'] + '\n')
                    qp_pair['para_sync_tokens'] = dict[id]['co_para']
                    qp_pair['para_sync_tokens'] = [i + len(attribute['question_tokens']) for i in
                                                    qp_pair['para_sync_tokens']]
                    fout.write(str(qp_pair['para_sync_tokens']) + ' ')
                    for idx in dict[id]['co_para']:
                    ...
                    example_count += 1
        return coreference, example_count


def expand_sync(ans_token_pos, context, answer):
    answer_len = len(answer.split())
    for i in range(1, 20):
        if ans_token_pos - i >= 0:
            try:
                if context[ans_token_pos - i].lower() in ['he', 'she']:
                    return ans_token_pos - i
            except:
                return -1
        if ans_token_pos + answer_len + i < len(context):
            try:
                if context[ans_token_pos + answer_len + i] in ['he', 'she']:
                    return ans_token_pos + answer_len + i
            except:
                return -1
    return -1


def convert_data(data_file, read_data_file, save_file, coreference, example_count):
    with open(data_file, 'r', encoding='utf-8') as f:
        with open(read_data_file, 'a+', encoding='utf-8') as w:
            ...
            for para in tqdm(data):
                ...
                query_index_expand, para_index_expand = expand_sync(para['question'], para['paragraph'], query_index[0],
                                                                    para_index[0])
            ...
            with open(save_file, 'w', encoding='utf-8') as fout:
                json.dump(coreference, fout)
    return example_count


def convert_data_all(data_file, read_data_file, save_file, coreference, example_count):
    with open(data_file, 'r', encoding='utf-8') as f:
        with open(read_data_file, 'a+', encoding='utf-8') as w:
            data = json.load(f)['data']
            example_count = 0
            for para in tqdm(data):
                if para['paragraphs']:
                    if para['paragraphs'][0]['doc_tokens_query'][0] != 'who':
                        continue
                    p = para['paragraphs'][0]
                    qa = p['qas'][0]
                    qp_pair = {}
...
if __name__ == '__main__':
    ...
    coreference, example_count = convert_data_100(args.data_file, args.read_data_file)
    example_count = convert_data_all(args.sync_file, args.read_data_file, args.save_file, coreference, example_count)
